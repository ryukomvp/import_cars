...
EJECUTAR_FORMULARIO.addEventListener('submit', async (event) => {
    ...
    (document.getElementById('id').value) ? action = 'actualizarMoneda' : action = 'crearMoneda';
    // Constante tipo objeto con los datos del formulario.
    const FORM = new FormData(EJECUTAR_FORMULARIO);
    // Petición para guardar los datos del formulario.
    const JSON = await dataFetch(MONEDA_API, action, FORM);
...
async function rellenarTabla(form = null) {
    ...
    (form) ? action = 'buscarMoneda' : action = 'leerMonedas';
            ...
            REGISTROS_TABLA.innerHTML += `
                <tr  class="text-center bg-white hover:bg-blue-200">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <button onclick="eliminarRegistro(${row.idmoneda})"
...
function crearRegistro() {
    ...
    TITULO.textContent = 'Añadir un nuevo tipo de moneda';
}
...
async function actualizarRegistro(id) {
    ...
    const JSON = await dataFetch(MONEDA_API, 'leerUnaMoneda', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (JSON.status) {
        // Se abre la caja de diálogo que contiene el formulario.
        ABRIR_MODAL.show();
        ...
        TITULO.textContent = 'Actualizar moneda';
        // Se inicializan los campos del formulario.
        document.getElementById('id').value = JSON.dataset.idmoneda;
        document.getElementById('moneda').value = JSON.dataset.moneda;
    } else {
        sweetAlert(2, JSON.exception, false);
    }
}
...
async function eliminarRegistro(id) {
    ...
    const MENSAJE = await confirmAction('¿Desea eliminar el tipo de moneda?');
    // Se verifica la respuesta del mensaje.
    if (MENSAJE) {
        // Se define una constante tipo objeto con los datos del registro seleccionado.
        const FORM = new FormData();
        FORM.append('idmoneda', id);
        // Petición para eliminar el registro seleccionado.
        const JSON = await dataFetch(MONEDA_API, 'eliminarMoneda', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (JSON.status) {
            // Se carga nuevamente la tabla para visualizar los cambios.
            rellenarTabla();
