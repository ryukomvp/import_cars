...
EJECUTAR_FORMULARIO.addEventListener('submit', async (event) => {
    ...
    const JSON = await dataFetch(PARAMETRO_API, action, FORM);
        ...
        JSON.dataset.forEach(row => {
            ...
            REGISTROS_TABLA.innerHTML += `
                <tr class="text-center bg-white hover:bg-blue-200">
                    ...
                    <td class="px-6 py-4">
                        <button onclick="actualizarRegistro(${row.idparametro})"
                        ...
                        <button onclick="eliminarRegistrote(${row.idparametro})"
...
function crearRegistro() {
    ...
    fillSelect(CONTACTO_API, 'leerRegistros', 'contacto');
}
...
async function actualizarRegistro(id) {
    ...
    const JSON = await dataFetch(PARAMETRO_API, 'leerUnRegistro', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (JSON.status) {
        // Se abre la caja de diálogo que contiene el formulario.
        ACCIONES_MODAL.show();
        ...
        document.getElementById('id').value = JSON.dataset.idparametro;
        document.getElementById('nombreEmp').value = JSON.dataset.nombreemp;
        document.getElementById('direccionEmp').value = JSON.dataset.direccionemp;
        document.getElementById('porcentaje').value = JSON.dataset.porcentaje;
        document.getElementById('registro').value = JSON.dataset.registro;
        document.getElementById('giro').value = JSON.dataset.giroempresa;
        document.getElementById('nit').value = JSON.dataset.nit;
        document.getElementById('dui').value = JSON.dataset.dui;
        fillSelect(CONTACTO_API, 'leerRegistros', 'contacto', JSON.dataset.idcontacto);
    } else {
        sweetAlert(2, JSON.exception, false);
    }
}
...
async function eliminarRegistrote(id) {
    ...
    const RESPONSE = await confirmAction('¿Desea eliminar el parametro de forma permanente?');
    // Se verifica la respuesta del mensaje.
    if (RESPONSE) {
        // Se define una constante tipo objeto con los datos del registro seleccionado.
        const FORM = new FormData();
        FORM.append('idparametro', id);
        // Petición para eliminar el registro seleccionado.
        const JSON = await dataFetch(PARAMETRO_API, 'eliminarRegistro', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (JSON.status) {
            // Se carga nuevamente la tabla para visualizar los cambios.
            cargarRegistros();
