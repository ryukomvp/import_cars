...
// Constante para la ruta del business que conecta a los metodos del SCRUD
const CODIGO_PLAZOS_API = 'business/codigosPlazos.php';
...
EJECUTAR_FORMULARIO.addEventListener('submit', async (event) => {
    ...
    const JSON = await dataFetch(PLAZOS_API, action, FORM);
        ...
        JSON.dataset.forEach(row => {
            ...
            REGISTROS_TABLA.innerHTML += `
                <tr  class="text-center bg-white hover:bg-blue-200">
                    <td class="hidden px-6 py-4">${row.idplazo}</td>
                    <td class="px-6 py-4">${row.descripcion}</td>
                    <td class="px-6 py-4">${row.vencimiento}</td>
                    <td class="px-6 py-4">${row.plazo}</td>
                    <td class="px-6 py-4">${row.tipoplazo}</td>
                    <td class="px-6 py-4">
                        <button onclick="actualizarRegistro(${row.idplazo})"
                        ...
                        <button onclick="eliminarRegistro(${row.idplazo})"
...
async function actualizarRegistro(id) {
    ...
    const JSON = await dataFetch(PLAZOS_API, 'leerUnRegistro', FORM);
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (JSON.status) {
        // Se abre la caja de diálogo que contiene el formulario.
        ABRIR_MODAL.show();
...
async function eliminarRegistro(id) {
    ...
    const MENSAJE = await confirmAction('¿Desea eliminar el plazo?');
    // Se verifica la respuesta del mensaje.
    if (MENSAJE) {
        // Se define una constante tipo objeto con los datos del registro seleccionado.
        const FORM = new FormData();
        FORM.append('idplazo', id);
        // Petición para eliminar el registro seleccionado.
        const JSON = await dataFetch(PLAZOS_API, 'eliminarRegistro', FORM);
        // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
        if (JSON.status) {
            // Se carga nuevamente la tabla para visualizar los cambios.
            cargarRegistros();
