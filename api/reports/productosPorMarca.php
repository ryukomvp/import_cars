    ...
    if ($marcas->setId($_GET['idmarca'])) {
        ...
        if ($rowMarcas = $marcas->leerUnRegistro()) {
            ...
            if ($dataProductos = $marcas->productosMarca()) {
                ...
                $pdf->cell(46.5, 10, 'Tipo', 1, 0, 'C', 1);
                $pdf->cell(46.5, 10, $pdf->encodeString('Categoría'), 1, 0, 'C', 1);
                $pdf->cell(46.5, 10, 'Modelo', 1, 1, 'C', 1);
                ...
                foreach ($dataProductos as $rowProducto) {
                    ...
                    $pdf->cell(46.5, 10, $rowProducto['tipoproducto'], 1, 0);
                    $pdf->cell(46.5, 10, $pdf->encodeString($rowProducto['categoria']), 1, 0);
                    $pdf->cell(46.5, 10, $pdf->encodeString($rowProducto['modelo']), 1, 1);
                }
            } else {
                $pdf->cell(0, 10, $pdf->encodeString('No hay productos para el país seleccionado'), 1, 1);
            }
        ...
        print('País incorrecta');
    }
} else {
    print('Debe seleccionar un país');
}
